...
app_bt_management_callback(wiced_bt_management_evt_t event,
    ...
    case BTM_ENABLED_EVT:
        ...
        if (WICED_BT_SUCCESS == p_event_data->enabled.status) {
            ...
            cy_log_msg(
                CYLF_DEF, CY_LOG_ERR,
                "Failed to initialize Bluetooth controller and stack \r\n");
        }
    ...
    case BTM_USER_CONFIRMATION_REQUEST_EVT:
        cy_log_msg(
            CYLF_DEF, CY_LOG_NOTICE,
            "* TM_USER_CONFIRMATION_REQUEST_EVT: Numeric_value = %" PRIu32
            " *\r",
    ...
    case BTM_PAIRING_COMPLETE_EVT:
        cy_log_msg(
            CYLF_DEF, CY_LOG_NOTICE, "  Pairing Complete: %d ",
    ...
    case BTM_ENCRYPTION_STATUS_EVT:
        ...
        cy_log_msg(CYLF_DEF, CY_LOG_NOTICE,
                   "  Encryption Status Event for : bd ");
    ...
    case BTM_BLE_CONNECTION_PARAM_UPDATE:
        ...
        cy_log_msg(CYLF_DEF, CY_LOG_NOTICE,
                   "ble_connection_param_update.bd_addr: ");
        print_bd_address(p_event_data->ble_connection_param_update.bd_addr);
        cy_log_msg(CYLF_DEF, CY_LOG_NOTICE,
                   "ble_connection_param_update.conn_interval       : %d\r\n",
                   p_event_data->ble_connection_param_update.conn_interval);
        cy_log_msg(CYLF_DEF, CY_LOG_NOTICE,
                   "ble_connection_param_update.conn_latency        : %d\r\n",
                   p_event_data->ble_connection_param_update.conn_latency);
        cy_log_msg(
            CYLF_DEF, CY_LOG_NOTICE,
            "ble_connection_param_update.supervision_timeout : %d\r\n",
            p_event_data->ble_connection_param_update.supervision_timeout);
        cy_log_msg(CYLF_DEF, CY_LOG_NOTICE,
                   "ble_connection_param_update.status              : %d\r\n\n",
...
static void app_bt_init(void) {
    ...
    if (CY_RSLT_SUCCESS != cy_result) {
        cy_log_msg(CYLF_DEF, CY_LOG_ERR,
                   "Advertisement LED PWM Initialization has failed! \r\n");
        CY_ASSERT(0);
    }
    ...
    if (WICED_BT_SUCCESS != result) {
        cy_log_msg(CYLF_DEF, CY_LOG_ERR,
                   "Advertisement cannot start because of error: %d \r\n",
                   result);
        CY_ASSERT(0);
    }
    ...
    cy_log_msg(CYLF_DEF, CY_LOG_INFO,
               "***********************************************\r\n");
    cy_log_msg(CYLF_DEF, CY_LOG_INFO,
               "**Discover device with \"Battery Server\" name*\r\n");
    cy_log_msg(CYLF_DEF, CY_LOG_INFO,
               "***********************************************\r\n\n");
}
/*
        ...
        if (battery_server_context.bt_conn_id) {
            if (app_bas_battery_level_client_char_config[0] &
                ...
                cy_log_msg(
                    ...
                    "================================================\r\n");
                cy_log_msg(CYLF_DEF, CY_LOG_NOTICE,
                           "Sending Notification: Battery level: %u\r\n",
                           app_bas_battery_level[0]);
                cy_log_msg(
                    CYLF_DEF, CY_LOG_NOTICE,
                    "================================================\r\n");
    ...
    case GATT_ATTRIBUTE_REQUEST_EVT:
        ...
        if (status != WICED_BT_GATT_SUCCESS) {
            wiced_bt_gatt_server_send_error_rsp(
                p_attr_req->conn_id, p_attr_req->opcode, error_handle, status);
        }
...
app_bt_connect_event_handler(wiced_bt_gatt_connection_status_t *p_conn_status) {
    ...
    if (NULL != p_conn_status) {
        if (p_conn_status->connected) {
            ...
            memcpy(battery_server_context.bt_peer_addr, p_conn_status->bd_addr,
                   BD_ADDR_LEN);
...
app_bt_server_event_handler(wiced_bt_gatt_event_data_t *p_data,
    ...
    case GATT_REQ_READ_BLOB:
        ...
        status = app_bt_gatt_req_read_handler(
            ...
            p_att_req->len_requested, p_error_handle);
        break;
    ...
    case GATT_HANDLE_VALUE_CONF: /* Value confirmation */
        ...
        if ((ota_lib_state ==
             CY_OTA_STATE_OTA_COMPLETE) && /* Check if we completed the download
                                              before rebooting */
            ...
            cy_ota_agent_stop(
                &battery_server_context.ota_context); /* Stop OTA */
        }
        status = WICED_BT_GATT_SUCCESS;
        break;
    ...
    case GATT_HANDLE_VALUE_NOTIF:
        cy_log_msg(CYLF_DEF, CY_LOG_NOTICE,
                   "  %s() GATT_HANDLE_VALUE_NOTIF - Notification Sent\r\n",
                   __func__);
        status = WICED_BT_GATT_SUCCESS;
        break;
...
app_bt_write_handler(wiced_bt_gatt_event_data_t *p_data,
    ...
    *p_error_handle = p_write_req->handle;
    ...
    case HDLC_OTA_FW_UPGRADE_SERVICE_OTA_UPGRADE_DATA_VALUE:
        ...
        return (result == CY_RSLT_SUCCESS) ? WICED_BT_GATT_SUCCESS
                                           : WICED_BT_GATT_ERROR;
            ...
            if (app_gatt_db_ext_attr_tbl[i].max_len >= len) {
                ...
                if (app_gatt_db_ext_attr_tbl[i].handle ==
                    ...
                    if (GATT_CLIENT_CONFIG_NOTIFICATION ==
                        ...
                        cy_log_msg(CYLF_DEF, CY_LOG_NOTICE,
                                   "Battery Server Notifications Enabled \r\n");
                    } else {
                        cy_log_msg(
                            CYLF_DEF, CY_LOG_NOTICE,
                            "Battery Server Notifications Disabled \r\n");
                    }
                }
            } else {
...
static void app_bt_adv_led_update(void) {
    ...
    if (CY_RSLT_SUCCESS != cy_result) {
        cy_log_msg(CYLF_DEF, CY_LOG_ERR,
                   "Failed to set duty cycle parameters!!\r\n");
    }
...
app_bt_gatt_req_read_handler(uint16_t conn_id, wiced_bt_gatt_opcode_t opcode,
    ...
    *p_error_handle = p_read_req->handle;
    ...
    if ((puAttribute = app_bt_find_by_handle(p_read_req->handle)) == NULL) {
        cy_log_msg(CYLF_DEF, CY_LOG_ERR,
                   "%s()  Attribute not found, Handle: 0x%04x\r\n", __func__,
                   p_read_req->handle);
...
static wiced_bt_gatt_status_t app_bt_gatt_req_read_by_type_handler(
    ...
    while (WICED_TRUE) {
        *p_error_handle = attr_handle;
        ...
        if ((puAttribute = app_bt_find_by_handle(attr_handle)) == NULL) {
            cy_log_msg(CYLF_DEF, CY_LOG_ERR,
                       "%s()  found type but no attribute for %d \r\n",
    ...
    if (used == 0) {
        cy_log_msg(CYLF_DEF, CY_LOG_ERR,
                   "%s()  attr not found  start_handle: 0x%04x  "
