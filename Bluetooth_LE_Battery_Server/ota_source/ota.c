 ...
 * the client device and invokes GATT Server Event Callback function. This
...
app_bt_ota_write_handler(wiced_bt_gatt_event_data_t *p_data,
    ...
    *p_error_handle = p_write_req->handle;
    ...
    case HDLD_OTA_FW_UPGRADE_SERVICE_OTA_UPGRADE_CONTROL_POINT_CLIENT_CHAR_CONFIG:
        cy_log_msg(CYLF_OTA, CY_LOG_DEBUG,
                   "%s() "
                   "HDLD_OTA_FW_UPGRADE_SERVICE_OTA_UPGRADE_CONTROL_POINT_"
                   "CLIENT_CHAR_CONFIG\r\n",
                   __func__);
        /* Save Configuration descriptor in Application data structure (Notify &
         * Indicate flags) */
        battery_server_context.bt_ota_config_descriptor = p_write_req->p_val[0];
        cy_log_msg(CYLF_OTA, CY_LOG_NOTICE,
                   "battery_server_context.bt_ota_config_descriptor: %d %s\r\n",
    ...
    case HDLC_OTA_FW_UPGRADE_SERVICE_OTA_UPGRADE_CONTROL_POINT_VALUE:
        ...
        case CY_OTA_UPGRADE_COMMAND_DOWNLOAD:
            ...
            cy_log_msg(CYLF_OTA, CY_LOG_DEBUG,
                       "%s() "
                       "HDLC_OTA_FW_UPGRADE_SERVICE_OTA_UPGRADE_CONTROL_POINT_"
                       "VALUE : CY_OTA_UPGRADE_COMMAND_DOWNLOAD\r\n",
                       __func__);
            cy_result = cy_ota_ble_download(
                battery_server_context.ota_context, p_data,
                battery_server_context.bt_conn_id,
                battery_server_context.bt_ota_config_descriptor);
            if (CY_RSLT_SUCCESS != cy_result) {
                cy_log_msg(CYLF_OTA, CY_LOG_ERR,
                           "Download Failed - result: 0x%lx\r\n", cy_result);
                return WICED_BT_GATT_ERROR;
            }
            return WICED_BT_GATT_SUCCESS;

        case CY_OTA_UPGRADE_COMMAND_VERIFY:
            cy_result = cy_ota_ble_download_verify(
                battery_server_context.ota_context, p_data,
                battery_server_context.bt_conn_id);
            if (CY_RSLT_SUCCESS != cy_result) {
                cy_log_msg(CYLF_OTA, CY_LOG_ERR,
                           "verification and Indication failed: 0x%d\r\n",
                           status);
                return WICED_BT_GATT_ERROR;
            }
            return status;
        ...
        case CY_OTA_UPGRADE_COMMAND_ABORT:
            cy_result =
                cy_ota_ble_download_abort(battery_server_context.ota_context);
            return WICED_BT_GATT_SUCCESS;
        }
        break;

    case HDLC_OTA_FW_UPGRADE_SERVICE_OTA_UPGRADE_DATA_VALUE:
        cy_result = cy_ota_ble_download_write(
            battery_server_context.ota_context, p_data);
        return (cy_result == CY_RSLT_SUCCESS) ? WICED_BT_GATT_SUCCESS
                                              : WICED_BT_GATT_ERROR;
...
cy_rslt_t app_bt_ota_init(app_context_t *app_context) {
    ...
    if (CY_RSLT_SUCCESS != cy_result) {
        cy_log_msg(CYLF_OTA, CY_LOG_ERR,
                   "cy_ota_agent_start() Failed - result: 0x%lx\r\n",
                   cy_result);
        while (true) {
